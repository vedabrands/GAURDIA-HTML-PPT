...
export const ActionSection: React.FC = () => {
  ...
  return (
    <section className="relative w-full min-h-screen bg-[#F5EFEB] text-[#12161A] p-4 sm:p-8 md:p-12 flex flex-col justify-between overflow-hidden border-b-4 border-[#B81D13] shadow-2xl">
      ...
      {/* 4 Core Pillars Grid */}
      <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 my-8">
        {cards.map((card, idx) => {
          ...
          return (
            <BlurReveal key={card.id} delay={0.1 + idx * 0.08} yOffset={25}>
              <motion.div
                ...
                className={`relative rounded-2xl p-5 cursor-pointer transition-all duration-300 flex flex-col justify-between h-full group border-2 ${
                  isHovered
              ...
              >
                ...
                <div>
                  ...
                  <h3
                    className={`font-display text-2xl uppercase tracking-wide leading-tight mb-2 transition-colors duration-200 ${
      ...
      <BlurReveal delay={0.25} yOffset={30}>
        <motion.div
          ...
          className="relative z-10 bg-[#141920] hover:bg-[#181F28] rounded-2xl p-6 text-white border-2 border-[#232B36] hover:border-[#B81D13] shadow-xl hover:shadow-[0_25px_45px_-10px_rgba(184,29,19,0.35)] transition-all duration-300"
        >
          <div className="flex items-center justify-between flex-wrap gap-4 mb-4 pb-4 border-b border-[#2A3441]">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-[#B81D13] flex items-center justify-center text-white font-bold shadow-[0_0_15px_rgba(184,29,19,0.4)]">
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {s.status ? (
                      <CheckCircle className={`w-4 h-4 ${isHovered ? 'text-white' : 'text-emerald-400'}`} />
                    ) : (
                      <span
